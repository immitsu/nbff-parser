/**
 * Currently only for `parse`,
 */

import { stringifyAttrs } from './stringify-attrs.js'
...
const prepareItem = ({ title, description, pid, id, items, ...rest }) => ({
  title,
  description,
  attrs: stringifyAttrs(rest)
})
...
export const stringify = folder =>
  template(folder.title, createContent(folder.items))
